...
#include "zdodcoincontroldialog.h"
#include "ui_zdodcoincontroldialog.h"
...
std::list<std::string> ZDODcoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDODcoinControlDialog::listMints;

ZDODcoinControlDialog::ZDODcoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDODcoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDODcoinControlDialog::~ZDODcoinControlDialog()
{
    delete ui;
}

void ZDODcoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDODcoinControlDialog::updateList()
{
...
void ZDODcoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDODcoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDODcoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDODcoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDODcoinControlDialog::GetSelectedMints()
{
...
void ZDODcoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

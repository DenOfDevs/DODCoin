...
void PrivacyDialog::on_pushButtonZDODcoinControl_clicked()
{
    ZDODcoinControlDialog* zDODControl = new ZDODcoinControlDialog(this);
    zDODControl->setModel(walletModel);
    zDODControl->exec();
}

void PrivacyDialog::setZDODcoinControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzDOD()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid DODcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DODCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    if (!ZDODcoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDODcoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zdodcoin selector in case it was used
    ZDODcoinControlDialog::listSelectedMints.clear();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " DODcoin, ";
